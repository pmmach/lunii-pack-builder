...
- **Intro audio** (réglage commun au pack) : choix entre
  - **Synthétique** (défaut) : synthèse vocale (TTS cloud) du **titre** de chaque histoire (et du titre du pack en multi-histoires), voix enfant française configurable via variables d'environnement — voir `specs/07-intro-tts.md`
  - **Découpée** : les N premières secondes du contenu découpé de chaque histoire (0 = pas d'intro)
- **Gestion de l'image** :
