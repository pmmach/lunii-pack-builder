...
3. **Découverte depuis une page HTML** (`src/lib/sources/generic-page.ts`) :
   ...
   - Chercher dans l'ordre :
     1. `<link rel="alternate" type="application/rss+xml" href>` (idem Atom)
     2. **URL de flux embarquée** dans la page (attributs `href` / `src` / texte) vers un hébergeur podcast connu (`feed.ausha.co`, `feeds.acast.com`, `rss.buzzsprout.com`, `feeds.megaphone.fm`, `feeds.transistor.fm`, `feeds.captivate.fm`, `anchor.fm`…/rss, `spreaker.com`…/feed, chemins `/feed.xml`, `/rss.xml`, `/podcast.xml`) — utile quand l'éditeur expose le flux sans balise `<link rel="alternate">`
     3. JSON-LD `@type` `PodcastSeries` / `PodcastEpisode` / `RadioEpisode` (`associatedMedia.contentUrl` / `contentUrl` / `mainEntity.contentUrl`, `image`)
     4. Balises `og:title` / `og:image` / `og:audio`
     5. **Liens d'annuaire** Apple Podcasts / Spotify / Deezer présents dans la page (`<a href>`, iframes) → déléguer à `resolveFromDirectory` (étape 4). Ordre de tentative : scorer chaque lien (similarité du slug/path avec le slug de la page et le titre `og:title`/`<h1>`/`<title>`, bonus Apple > Spotify > Deezer, bonus position précoce dans le document), essayer jusqu'à 3 candidats distincts. Couvre les pages éditeur type Bayard/Ausha qui n'exposent pas de balise RSS mais un bouton « écouter sur Apple / Spotify ».
     6. **Recherche iTunes par titre de page** : si rien n'a abouti, nettoyer `og:title` / `<h1>` / `<title>` (retirer les suffixes après ` - `, ` | `, ` · `, noms de site) et appeler `searchItunesFeed` ; si un match raisonnable existe → parser le flux, `resolvedFrom: "directory-search"`
   - Si un flux RSS est trouvé (étapes 1–2) → le parser avec `rss-parser`, `resolvedFrom: "page-discovery"`, puis **enrichir les images d'épisodes** depuis le HTML déjà récupéré via `enrichEpisodeImagesFromPage` (`src/lib/sources/page-images.ts`) : pour chaque épisode, chercher sur la page un nœud dont le texte normalisé correspond exactement au titre, remonter jusqu'à 8 ancêtres pour trouver une `<img>`/`<source srcset>` exploitable (hors logos/icônes `.svg`), et remplacer `imageUrl` uniquement si une image spécifique est trouvée — sinon conserver l'image issue du flux RSS. Utile quand le flux ne fournit qu'une pochette générique identique pour tous les items alors que la page liste une vignette par épisode (ex. pages émissions Radio France).
   ...
   - Les étapes 5–6 ne scrapent **jamais** l'audio des plateformes DRM : elles ne font que retrouver le flux RSS public de l'éditeur (même règle que l'étape 4)

4. **Résolution d'annuaire (Spotify / Apple Podcasts / Deezer)** (`src/lib/sources/directory-resolver.ts`), déclenchée si l'hôte matche `open.spotify.com`, `podcasts.apple.com`, `deezer.com` (URL collée directement **ou** lien découvert à l'étape 3.5) :
   - **Apple Podcasts avec `id` numérique** dans l'URL (`/id1457819198` ou `?id=…`) : appeler d'abord `https://itunes.apple.com/lookup?id=<id>&entity=podcast` et utiliser le `feedUrl` du résultat — plus fiable qu'une recherche textuelle
   - Sinon, récupérer le nom de l'émission :
...
- `generic-page.test.ts` : fixture `sample-page-with-link.html` (contient `<link rel="alternate" type="application/rss+xml">`), fixture `sample-page-jsonld.html` (JSON-LD sans lien RSS mais avec audio direct), fixture `sample-page-episode-cards.html` (page d'émission avec une vignette distincte par carte d'épisode) → vérifie que `resolveFromHtmlBody` associe l'image de la carte aux épisodes dont le titre matche et laisse le fallback flux pour les autres ; fixture `sample-page-directory-links.html` (pas de balise RSS, liens Apple + Spotify + shows connexes) → vérifie que le lien Apple le mieux scoré est suivi et que le flux est résolu (`resolvedFrom: "directory-search"`) ; fixture `sample-page-embedded-feed.html` (lien `feed.ausha.co` sans balise alternate) → `resolvedFrom: "page-discovery"` ; mocker `fetch` pour ne jamais toucher le réseau
- `directory-resolver.test.ts` : mocker les réponses `oembed`, `itunes search` et `itunes lookup`, vérifier le happy path, le lookup par ID Apple, et le cas `NO_PUBLIC_FEED` ; tests unitaires de `extractApplePodcastId`, `scoreDirectoryCandidate`, `cleanShowTitle`
...
- Coller `https://tout.bayard-jeunesse.com/podcasts/la-grande-histoire-de-pomme-d-api/` → retrouve le flux public Ausha (via lien Apple / Spotify sur la page ou recherche iTunes), liste les épisodes ; **pas** d'erreur `UNSUPPORTED_SOURCE`
