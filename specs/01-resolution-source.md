...
3. **Découverte depuis une page HTML** (`src/lib/sources/generic-page.ts`) :
   ...
   - Si un flux RSS est trouvé → le parser avec `rss-parser`, `resolvedFrom: "page-discovery"`
...
- `generic-page.test.ts` : fixture `sample-page-with-link.html` (contient `<link rel="alternate" type="application/rss+xml">`) et fixture `sample-page-jsonld.html` (JSON-LD sans lien RSS mais avec audio direct) ; mocker `fetch` pour ne jamais toucher le réseau
